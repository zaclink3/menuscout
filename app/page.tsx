...
function TopBar({
  ...
  return (
    <div className="sticky top-0 z-30 border-b bg-gradient-to-r from-primary/5 via-background to-background backdrop-blur supports-[backdrop-filter]:bg-background/70">
      ...
      <div className="max-w-6xl mx-auto px-4 py-3 flex flex-wrap gap-2 items-center">
        ...
        <Select value={category} onValueChange={(v) => setCategory(v)}>
          ...
          <SelectContent>
            ...
            {CATEGORY_TAXONOMY.map((c) => (<SelectItem key={c} value={c}>{titleCase(c)}</SelectItem>))}
          </SelectContent>
        </Select>
        <DropdownMenu>
          <DropdownMenuTrigger asChild><Button variant="outline" size="sm">Neighborhoods</Button></DropdownMenuTrigger>
            ...
            {neighborhoods.map((n) => (
              <DropdownMenuCheckboxItem
                ...
                onCheckedChange={(checked) => {
                  if (checked) setSelectedNeighborhoods((prev) => (Array.isArray(prev) && prev.includes(n) ? prev : [...prev, n]));
                  else setSelectedNeighborhoods((prev) => (Array.isArray(prev) ? prev.filter((x) => x !== n) : []));
                }}
              >
                {n}
              </DropdownMenuCheckboxItem>
...
function DealRow({ d }: { d: Deal }) {
  return (
    <div className="grid grid-cols-[1fr_auto] gap-3 items-center p-3 rounded-lg border bg-muted/30">
      <div className="space-y-1 min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          <Badge variant="secondary">{d.title}</Badge>
          {d.weekday ? (
            <span className="inline-flex items-center gap-1 text-muted-foreground text-sm whitespace-nowrap">
        ...
        {d.restrictions?.length ? (<p className="text-xs text-muted-foreground">Restrictions: {d.restrictions.join(", ")}</p>) : null}
        {d.source_url ? (
      ...
      <div className="text-right">
        {typeof d.price === "number" ? (
          <>
            <div className="text-lg font-semibold tabular-nums whitespace-nowrap">${d.price.toFixed(2)}</div>
            <div className="text-[10px] text-muted-foreground">{d.currency || "USD"}</div>
          </>
        ) : (
          <div className="text-xs text-muted-foreground">Varies</div>
        )}
      </div>
    </div>
  );
        ...
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {v.categories?.slice(0, 3).map((c) => (
              <Badge key={c} variant="outline" className="capitalize">{titleCase(c)}</Badge>
            ))}
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          {v.address?.street ? (
            <div className="text-sm text-muted-foreground flex items-center gap-2">
              ...
              <span>{v.address.street}, {v.address.city}, {v.address.region} {v.address.postal_code}</span>
            </div>
          ) : null}
          {v.deals?.length ? (
            <div className="space-y-2">
              {v.deals.map((d, i) => (<DealRow key={`${v.venue_name}-${i}`} d={d} />))}
            </div>
          ) : (
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader><DialogTitle>Import venues JSON</DialogTitle></DialogHeader>
        <div className="space-y-2">
...
export default function MenuScoutApp() {
  ...
  async function fetchVenues() {
    try {
      ...
      if (Array.isArray(data)) setVenues(data as Venue[]);
    } catch (e) {
          ...
          <TabsContent value="list" className="mt-4">
            {filtered.length ? (
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filtered.map((v) => (<VenueCard key={v.venue_name} v={v} />))}
              </div>
            ) : (
